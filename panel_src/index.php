...
<div class="wrap">
    ...
    <!-- Cloudflare WARP -->
    <div class="cols" id="warpSection" style="margin-top: 0;">
        <!-- Left: WARP Configuration -->
        <div class="card" id="warpControlCard">
            <div class="section-title">
                <span class="icon">🌀</span> Cloudflare WARP
            </div>
            
            <div id="warpUnregisteredView">
                <p style="color:var(--text-dim); margin-bottom: 20px;">
                    Cloudflare WARP не подключен. Вы можете автоматически зарегистрировать новое устройство в сети Cloudflare и использовать его для обхода блокировок или как основной канал выхода.
                </p>
                <button class="btn" id="btnWarpRegister" onclick="registerWarp()">Подключить Cloudflare WARP</button>
            </div>
            
            <div id="warpRegisteredView" style="display:none;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
                    <div>
                        <span style="font-weight:600; font-family:var(--font-heading);">Состояние подключения</span>
                        <div style="font-size:12px; color:var(--text-dim); margin-top:2px;" id="warpStatusText">Проверка...</div>
                    </div>
                    <button class="btn btn-outline btn-sm" id="btnWarpToggle" onclick="toggleWarp()">Включить</button>
                </div>
                
                <div class="fg">
                    <label for="selectWarpMode">Режим маршрутизации трафика</label>
                    <select id="selectWarpMode" onchange="setWarpMode(this.value)">
                        <option value="smart">Умный (Smart) — только OpenAI/Netflix/ipify</option>
                        <option value="all">Глобальный (Global) — весь трафик через WARP</option>
                    </select>
                </div>
                
                <div style="display:flex; gap:12px; margin-top: 24px; border-top:1px solid rgba(255,255,255,0.05); padding-top:16px;">
                    <button class="btn btn-outline btn-sm" onclick="registerWarp()">Переподключить</button>
                    <button class="btn btn-outline btn-sm" style="border-color:var(--err); color:var(--err);" onclick="deleteWarp()">Удалить подключение</button>
                </div>
            </div>
        </div>

        <!-- Right: WARP Metadata -->
        <div class="card" id="warpInfoCard">
            <div class="section-title">
                <span class="icon">📋</span> Информация о подключении
            </div>
            <div id="warpInfoContent" style="display:flex; flex-direction:column; gap:12px;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <span style="color:var(--text-dim);">Статус:</span>
                    <span class="tag tag-warn" id="warpStatusBadge">Не подключен</span>
                </div>
                <div class="warp-meta-item" style="display:none; justify-content:space-between; align-items:center;">
                    <span style="color:var(--text-dim);">Device ID:</span>
                    <code class="mono" id="warpDeviceId">—</code>
                </div>
                <div class="warp-meta-item" style="display:none; justify-content:space-between; align-items:center;">
                    <span style="color:var(--text-dim);">Локальный IPv4:</span>
                    <code class="mono" id="warpIPv4">—</code>
                </div>
                <div class="warp-meta-item" style="display:none; justify-content:space-between; align-items:center;">
                    <span style="color:var(--text-dim);">Локальный IPv6:</span>
                    <code class="mono" id="warpIPv6">—</code>
                </div>
                <div class="warp-meta-item" style="display:none; justify-content:space-between; align-items:center;">
                    <span style="color:var(--text-dim);">Лицензия:</span>
                    <code class="mono" id="warpLicense">—</code>
                </div>
            </div>
        </div>
    </div>
